...
Example:
    # Single run directory containing question_* subfolders
    python3 scripts/sample_qp_accuracy_new.py \
        --source-run output_aime/aime_Qwen3-32B_QP32_CP2_BS1_beam_search/Qwen3-32B/\
Skywork-o1-Open-PRM-Qwen-2.5-1.5B/40_4_2 \
        --sample-sizes 64,32,16,8,4,2 \
        --trials 3 \
        --seed 42 \
        --save sampled_results_aime.csv

# Wildcard across multiple runs (all matched directories processed)
    python3 scripts/sample_qp_accuracy_new.py \
        --source-run "output_aime/aime_Qwen3*" \
        --sample-sizes 64,32,16,8,4,2,1 \
        --trials 3 \
        --seed 42 \
        --save trials_aime.csv \
        --save-summary summary_aime.csv 
"""
...
def extract_candidate_tokens(out: Dict[str, Any]) -> int:
    """Pull completion token count from a single candidate dict."""
    for key in (
        "completion_tokens",
...
def get_question_length(qdir: str, token_lens: Dict[str, int]) -> Optional[int]:
    ...
    for key, val in token_lens.items():
        ...
        if key.endswith(suffix):
            ...
            # We look for exact word match or just substring? 
            # Substring is safer given directory naming variability.
            # e.g. "aime" in ".../aime_Qwen3-8B..." -> Yes.
...
def score_sample(
    ...
    for q_idx, (qdir, recs) in enumerate(items):
        ...
        # Calculate eFLOPs for the chosen sample if calculator is provided
        if eflops_calc:
            # Convert Path objects to strings for the calculator
            # Crucial Fix: map .jsonl files (summary) to _beam.json files (tree structure)
            beam_files = []
            for p in chosen:
                # Assuming pattern record_X.jsonl -> record_X_beam.json
                # Check if file exists
                beam_p = p.parent / p.name.replace(".jsonl", "_beam.json")
                if beam_p.exists():
                    beam_files.append(str(beam_p))
                else:
                    # Fallback: maybe the input was already _beam.json? or pattern differs
                    # But collect_question_records filters by RECORD_PATTERN (record_X.jsonl)
                    # So we just warn if missing
                    # print(f"Warning: Beam file not found for {p}")
                    pass
            ...
            if beam_files:
                try:
                    ...
                    # print(f"Error calculating eFLOPs for {qdir}: {e}")
                    pass
    ...
    for run_idx, info in enumerate(run_infos):
        ...
        for sample_size in info["sample_sizes"]:
            ...
            print(
                ...
                f"mean_eflops={mean_avg_eflops:.2e} mean_memory={mean_avg_memory:.2e}"
            )
            summary_rows.append(
                {
                    "序号": summary_index,
