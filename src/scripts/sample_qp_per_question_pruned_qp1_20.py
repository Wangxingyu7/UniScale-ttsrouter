...
Example:
    python3 scripts/sample_qp_per_question_pruned_qp1_20.py \
        --source-run "output_test150/*QP64*CP1*BS1*" \
        --sample-sizes 1-20 \
        --trials 3 \
        --seed 42 \
        --save qp1_to_qp20_trials_test150.csv \
        --save-summary qp1_to_qp20_summary_test150.csv
"""
...
def derive_sample_sizes(
    ...
    if qp is not None:
        limit = min(limit, qp)
    # Keep only explicitly requested sample sizes; do not auto-add power-of-two sizes.
...
def parse_sample_sizes(raw: str) -> List[int]:
    ...
    for chunk in raw.split(","):
        ...
        if not chunk:
            continue
        if "-" in chunk:
            start_str, end_str = [p.strip() for p in chunk.split("-", 1)]
            start = int(start_str)
            end = int(end_str)
            if start <= 0 or end <= 0:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="1-20",
        help=(
            "Target sample sizes. Supports comma list and ranges, e.g. '1-20' or '20,16,12,8'."
        ),
    )
    parser.add_argument(
        "--trials",
    ...
    for rd in run_dirs:
        ...
        if meta.get("qp") != 64 or meta.get("cp") != 1 or meta.get("bs") != 1:
            skipped_runs.append(f"{rd} (requires QP=64, CP=1 and BS=1 source run)")
            continue
        ...
        for question_index, (qdir, recs) in enumerate(sorted_questions):
            ...
            for sample_size in sample_sizes_for_run:
                ...
                summary_rows.append(
                    {
                        "序号": 0,
                        "question_model_qp_cp_bs": f"{question_name}_{model_prefix}_QP{sample_size}_CP1_BS1",
                        "qp": sample_size,
                        "cp": 1,
                        "bs": 1,
    ...
    if args.save:
        ...
        with out_path.open("w", newline="", encoding="utf-8") as fh:
            ...
            for idx, row in enumerate(rows_for_csv, start=1):
                output_row = {
                    "序号": idx,
                    "question_model_qp_cp_bs": f"{row.get('question', '')}_{row.get('model_prefix', '')}_QP{row.get('sample_size', '')}_CP1_BS1",
                    "qp": row.get("sample_size", ""),
                    "cp": 1,
                    "bs": 1,
                    "n_trials": 1,
