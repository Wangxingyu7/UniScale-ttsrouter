...
class CoTEnv(BaseEnv):
    ...
    def reset(self, update_legal_action=True):
        ...
        self.reward_history = []
        self.token_history = []
    # per-step latency history for this env (list of floats)
        self.step_latency_history = []
        # New latency tracking
        self.step_lm_latency_history = []  # LM latency for each step
        self.step_rm_latency_history = []  # RM latency for each step
        self.step_wait_history = []  # Wait time for each step
        import time
        self.question_start_time = time.time()  # Start time for current question
    ...
    def update_legal_actions(self, initial=False, force_update=False, custom_n=0):
        ...
        if len(self.llm_gen_fns) == 1:
            ...
            self.step_lm_latency_history.append(lm_latency)
            ...
            lm = self.llm_gen_fns[self.selected_model_idx]  # 关键修改：用选中的LM索引
            if initial:
                n = self.config["max_actions"]
            elif custom_n:
                n = custom_n
            ...
            import time
            lm_start_time = time.time()
            result: ConcatedLMGenResult = lm(  # 关键修改：用选中的LM调用生成
                messages=messages,
                config=LMCallingConfig(
                    n=n,
                    stop_str=stop_str,
            ...
            lm_latency = time.time() - lm_start_time
            self.step_lm_latency_history.append(lm_latency)
            texts = result.text  # 添加这行：定义texts变量
            logps_avg_by_len = result.logp_avg_by_len
            token_len = result.num_tokens
            temp_model_names = [lm.model_name] * len(texts)
            temp_model_ids = [self.selected_model_idx] * len(texts)
            finish_reason_list = []
            if isinstance(result.finish_reason, list):
                finish_reason_list.extend(result.finish_reason)
            else:
                raise ValueError("finish_reason should be a list")
            print(f"n: {finish_reason_list}")
    ...
    def copy(self):
        ...
        # copy step latency history
        env.step_latency_history = copy.deepcopy(self.step_latency_history)
        env.step_lm_latency_history = copy.deepcopy(self.step_lm_latency_history)
        env.step_rm_latency_history = copy.deepcopy(self.step_rm_latency_history)
        env.step_wait_history = copy.deepcopy(self.step_wait_history)
        env.question_start_time = copy.deepcopy(self.question_start_time)
        env.question_latency = copy.deepcopy(self.question_latency)
        return env
