"""
Controller for managing workers and dispatching requests.
This module adapts the key parts of FastChat's controller while removing the dependency
on the `fastchat` package. Use it to register workers, receive heartbeats, and dispatch
streaming generate requests to workers.
"""
...
# Local constants (replace fastchat.constants)
CONTROLLER_HEART_BEAT_EXPIRATION = int(os.environ.get("CONTROLLER_HEART_BEAT_EXPIRATION", 300))
WORKER_API_TIMEOUT = int(os.environ.get("WORKER_API_TIMEOUT", 30))
SERVER_ERROR_MSG = "Server Error"


class ErrorCode:
    CONTROLLER_NO_WORKER = 1001
    CONTROLLER_WORKER_TIMEOUT = 1002
...
class Controller:
    ...
    def register_worker(self, worker_name: str, check_heart_beat: bool, worker_status: dict, multimodal: bool):
        ...
        self.worker_info[worker_name] = WorkerInfo(
            worker_status.get("model_names", []),
            worker_status.get("speed", 1),
            worker_status.get("queue_length", 0),
            check_heart_beat,
            time.time(),
            multimodal,
        )
    ...
    def get_worker_address(self, model_name: str):
        if self.dispatch_method == DispatchMethod.LOTTERY:
            ...
            for w_name, w_info in self.worker_info.items():
                if model_name in w_info.model_names:
                    worker_names.append(w_name)
                    worker_qlen.append(w_info.queue_length / max(1, w_info.speed))
    ...
    def handle_no_worker(self, params):
        ...
        ret = {"text": SERVER_ERROR_MSG, "error_code": ErrorCode.CONTROLLER_NO_WORKER}
        return json.dumps(ret).encode() + b"\0"

    def handle_worker_timeout(self, worker_address):
        logger.info(f"worker timeout: {worker_address}")
        ret = {"text": SERVER_ERROR_MSG, "error_code": ErrorCode.CONTROLLER_WORKER_TIMEOUT}
        return json.dumps(ret).encode() + b"\0"
    ...
    def worker_api_get_status(self):
        ...
        for w_name in list(self.worker_info.keys()):
            worker_status = self.get_worker_status(w_name)
            if worker_status is not None:
                for mn in worker_status.get("model_names", []):
                    model_names.add(mn)
                speed += worker_status.get("speed", 1)
                queue_length += worker_status.get("queue_length", 0)

        model_names = sorted(list(model_names))
        return {"model_names": model_names, "speed": speed, "queue_length": queue_length}

    def worker_api_generate_stream(self, params):
        worker_addr = self.get_worker_address(params["model"]) if "model" in params else ""
        if not worker_addr:
...
async def register_worker(request: Request):
    ...
    controller.register_worker(
        data.get("worker_name") or data.get("worker_address") or "",
...
async def get_worker_address(request: Request):
    data = await request.json()
    addr = controller.get_worker_address(data.get("model", ""))
    return {"address": addr}
...
async def receive_heart_beat(request: Request):
    data = await request.json()
    exist = controller.receive_heart_beat(data.get("worker_name", ""), data.get("queue_length", 0))
    return {"exist": exist}
...
if __name__ == "__main__":
    ...
    if args.ssl:
        uvicorn.run(app, host=args.host, port=args.port, log_level="info", ssl_keyfile=os.environ.get("SSL_KEYFILE"), ssl_certfile=os.environ.get("SSL_CERTFILE"))
    else:
