...
class TTSRouter:
    ...
    async def run_evaluation(
        ...
        if not script_path.exists():
            raise HTTPException(status_code=500, detail=f"找不到脚本: {script_path}")

        cmd = [
            bash_cmd, str(script_path),
            "--method", eval_config.method,
            "--RM", config.default_rm,
            "--task_name", "input"
        ]
        ...
        try:
            ...
            try:
                ...
                with contextlib.suppress(ProcessLookupError):
                    proc.kill()
                raise HTTPException(status_code=504, detail=f"评估超时（>{timeout_seconds}s）")
    ...
    def _get_bash_command(self) -> str:
        ...
        raise HTTPException(
            status_code=400, 
            detail="未找到 bash：请使用 WSL/Git Bash 或设置环境变量 BASH_PATH 指向 bash 可执行文件"
        )
...
async def _process_evaluation_request(
    ...
    try:
        ...
        stdout, stderr, run_dir = await router.run_evaluation(
            task_id, eval_config, save_root_dir=save_root_dir
        )
